...
/************* PUBLIC APIS *************/
feedController.post("/feed/RepriceProduct/:id", async (req: Request, res: Response): Promise<any> => {
  ...
  if (productDetails && productDetails.activated == true) {
    const existingPrice = await responseUtility.GetLastExistingPrice(productDetails);
    //2.Check for eligibility
    const contextErrorItemsList: any[] = [];
    //await dbHelper.GetContextErrorItems(true);
    const isRepriceEligible = await responseUtility.IsEligibleForReprice(contextErrorItemsList, mpid);
    if (isRepriceEligible == true) {
      ...
      if (productDetails.competeAll == false) {
        sisterVendorItemDetails = await axiosHelper.GetSisterVendorItemDetails(mpid, globalParamInfo);
      }
      ...
      if (currentDetailsFromFeed) {
        if (currentDetailsFromFeed.availability == "in stock") {
          ...
          const floorPrice = productDetails.floorPrice ? productDetails.floorPrice : 0;
          ...
          if (offsetPrice < floorPrice) {
            repriceModel.generateRepriceData(existingPrice, 0 as unknown as string, false, RepriceRenewedMessageEnum.OFFSET_LESS_THAN_FLOOR);
          } else if (sisterVendorItemDetails.length > 0) {
            const sisterPriceComparedToLowest = sisterVendorItemDetails.find((x) => x.unitPrice == currentDetailsFromFeed.price);
            if (sisterPriceComparedToLowest) {
              repriceModel.generateRepriceData(existingPrice, 0 as unknown as string, false, RepriceRenewedMessageEnum.IGNORE_SISTER_VENDOR);
            } else {
              repriceModel.generateRepriceData(existingPrice, offsetPrice as unknown as string, true, RepriceRenewedMessageEnum.REPRICE_DEFAULT);
            }
          } else {
            repriceModel.generateRepriceData(existingPrice, offsetPrice as unknown as string, true, RepriceRenewedMessageEnum.REPRICE_DEFAULT);
          }
        } else {
          repriceModel.generateRepriceData(existingPrice, 0 as unknown as string, false, RepriceRenewedMessageEnum.IGNORE_NOT_IN_STOCK);
        }
      } else {
        repriceModel.generateRepriceData(existingPrice, 0 as unknown as string, false, RepriceRenewedMessageEnum.IGNORE_NOT_FOUND_API);
      }
    }
    // Apply Reprice Rule for Product
    if (productDetails.repricingRule != null && productDetails.isOverrideRun != true) {
      repriceModel = Rule.ApplyRule(repriceModel, productDetails.repricingRule, undefined, undefined);
    }

    // Update History
    await HistoryHelper.Execute(mpid, repriceModel, [feedOutput], false, undefined, undefined);

    let outputResponse = new RepriceAsyncResponse(repriceModel, feedOutput);

    //7.Update Product details in MongoDb
    const repriceNeeded = await isPriceUpdateRequired(repriceModel, productDetails.allowReprice);
    if (repriceNeeded == true) {
      ...
      priceUpdatedRequest.payload = new UpdateRequest(mpid, repriceModel.repriceDetails?.newPrice, 1, undefined);
      const priceUpdateUrl = apiMapping.find((x) => x.vendor === repriceModel.vendorName.toUpperCase())?.priceUpdateUrl;
      priceUpdatedResponse = await axiosHelper.postAsync(priceUpdatedRequest, priceUpdateUrl!);
      if (priceUpdatedResponse.data) {
        if (priceUpdatedResponse.data.message && (JSON.stringify(priceUpdatedResponse.data.message).indexOf("ERROR:422") > -1 || JSON.stringify(priceUpdatedResponse.data.message).indexOf("429") > -1)) {
  ...
  return res.status(_codes.StatusCodes.IM_A_TEAPOT).json("Invalid response found");
});

/************* PRIVATE FUNCTIONS *************/
async function isPriceUpdateRequired(repriceResult: any, isRepriceOn: any) {
  if (isRepriceOn == true && repriceResult.repriceDetails && repriceResult.repriceDetails.newPrice != "N/A" && repriceResult.repriceDetails.newPrice != repriceResult.repriceDetails.oldPrice) {
    return true;
  } else if (isRepriceOn == true && repriceResult.isMultiplePriceBreakAvailable == true && repriceResult.listOfRepriceDetails.length > 0) {
    let $eval = false;
