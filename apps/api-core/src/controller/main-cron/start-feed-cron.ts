...
async function repriceFeed(keyGen: any, productList: any, cronInitTime: any) {
  ...
  let _contextCronStatus = new CronStatusModel(cronInitTime, 0, _.filter(productList, (p) => p.scrapeOn == true && p.activated == true).length, "In-Progress", feedName, keyGen);
  ...
  for (let prod of productList) {
    ...
    if (prod.scrapeOn == true) {
      ...
      if (repriceResult && repriceResult.data) {
        if (repriceResult.data.priceUpdateResponse && repriceResult.data.priceUpdateResponse != null) {
          if (JSON.stringify(repriceResult.data.priceUpdateResponse).indexOf("ERROR:422") == -1) {
            cronLogs.logs.push({
              productId: prod.mpid,
            ...
            prod.next_cron_time = getNextCronTime(repriceResult.data.priceUpdateResponse);
      ...
      } else if (repriceResult && (repriceResult.status == _codes.StatusCodes.BAD_REQUEST || repriceResult.status == _codes.StatusCodes.IM_A_TEAPOT)) {
        cronLogs.logs.push({
          productId: prod.mpid,
      ...
      prod.last_cron_message = await filterMapper.GetLastCronMessage(repriceResult, prod.mpid, "UNKNOWN");
