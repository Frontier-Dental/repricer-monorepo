...
export async function updateProductManualHandler(req: Request, res: Response): Promise<any> {
  ...
  if (resultant) {
    if (resultant.priceUpdateResponse && resultant.priceUpdateResponse != null) {
      if (JSON.stringify(resultant.priceUpdateResponse).indexOf("ERROR:422") == -1 && JSON.stringify(resultant.priceUpdateResponse).indexOf("ERROR:429") == -1 && JSON.stringify(resultant.priceUpdateResponse).indexOf("ERROR:404") == -1 && JSON.stringify(resultant.priceUpdateResponse).indexOf("ERROR:") == -1) {
    ...
    prod.last_cron_message = await filterMapper.GetLastCronMessage(tempData, prod.mpid, "UNKNOWN");
  }
