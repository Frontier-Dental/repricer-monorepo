...
export async function repriceProductToMax(req: Request, res: Response): Promise<any> {
  ...
  if (!result || result.length === 0) {
    return res.status(_codes.StatusCodes.IM_A_TEAPOT).json("Invalid response found in Net32 Api");
  }
  ...
  if (!ownProduct) {
    return res.status(_codes.StatusCodes.BAD_REQUEST).json("Could not find own vendor Id");
  }

  if (productItem && ownProduct && ownProduct.inStock == true) {
    if (ownProduct.priceBreaks) {
      repriceResult = await repriceHelper.RepriceToMax(ownProduct, output, productItem, req.params.id);
    }
  } else {
    repriceResult = new RepriceModel(req.params.id, ownProduct, productItem.productName, productItem.unitPrice, false, false, [], RepriceMessageEnum.IGNORE_PRODUCT_INACTIVE);
  }

  if (!repriceResult) {
    return res.status(_codes.StatusCodes.BAD_REQUEST).json("Could not find repriceResult");
  }

  // Update History
  await HistoryHelper.Execute(req.params.id, repriceResult, output, false, contextVendor);
  ...
  const priceUpdateUrl = apiMapping.find((x) => x.vendor == contextVendor.toUpperCase())?.priceUpdateUrl;
  let priceUpdatedResponse = null;
  if (repriceResult && repriceResult.isMultiplePriceBreakAvailable != true) {
    priceUpdatedRequest.payload = new UpdateRequest(req.params.id, repriceResult.repriceDetails!.newPrice, 1, productItem.cronName);
  ...
  if (priceUpdatedResponse && priceUpdatedResponse.data) {
    if (priceUpdatedResponse.data.message && (JSON.stringify(priceUpdatedResponse.data.message).indexOf("ERROR:422") > -1 || JSON.stringify(priceUpdatedResponse.data.message).indexOf("ERROR:429") > -1 || JSON.stringify(priceUpdatedResponse.data.message).indexOf("ERROR:404") > -1 || JSON.stringify(priceUpdatedResponse.data.message).indexOf("ERROR:") > -1)) {
      ...
      if (outputResponse.repriceData && outputResponse.repriceData.isMultiplePriceBreakAvailable == false && outputResponse.repriceData.repriceDetails) {
        if (outputResponse.repriceData.repriceDetails!.explained!.indexOf("#NEW") < 0) {
