...
export function startV2AlgoHtmlFileCleanupCron(): void {
  if (cleanupCron) {
    logger.info("Cleanup cron is already running");
    return;
  }
  ...
  cleanupCron = schedule(
    ...
    async () => {
      try {
        logger.info(`Starting cleanup of expired v2_algo_execution records at ${new Date().toISOString()}`);
        await cleanupExpiredRecords();
        logger.info(`Cleanup completed successfully at ${new Date().toISOString()}`);
      } catch (error) {
        logger.error("Error during cleanup cron execution:", error);
      }
    },
    {
      scheduled: true,
  ...
  logger.info("Cleanup cron job started successfully. Running every 2 hours.");
}
...
export async function runCleanupManually(): Promise<void> {
  try {
    logger.info("Starting manual cleanup of expired v2_algo_execution records");
    await cleanupExpiredRecords();
...
async function cleanupExpiredRecords(): Promise<void> {
  ...
  const deletedCount = await knex("v2_algo_execution").where("expires_at", "<", currentTime).del();
  //destroyKnexInstance();
  logger.info(`Deleted ${deletedCount} expired records from v2_algo_execution table`);
}
...
export async function getExpiredRecordsCount(): Promise<number> {
  ...
  try {
    const count = await knex("v2_algo_execution").where("expires_at", "<", currentTime).count("* as count").first();

    return count ? Number(count.count) : 0;
  } catch (error) {
    logger.error("Error getting expired records count:", error);
    throw error;
  } finally {
    //destroyKnexInstance();
  }
...
export async function getTotalRecordsCount(): Promise<number> {
  ...
  try {
    ...
    logger.error("Error getting total records count:", error);
    throw error;
  } finally {
    //destroyKnexInstance();
  }
