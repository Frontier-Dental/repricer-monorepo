...
export async function parseShippingBuyBox(repriceResult: any, net32Result: any, productItem: any): Promise<any> {
  ...
  if (buyBoxVendor && buyBoxVendor.vendorId.toString() == productItem.ownVendorId.toString()) {
    return new RepriceModel(productItem.mpid, ownVendorItem, productItem.productName, "N/A", false, false, [], RepriceRenewedMessageEnum.DEFAULT);
    ...
    try {
      ...
      if (buyBoxVendorShippingTime == 1 && (ownVendorShippingTime == 2 || ownVendorShippingTime == 1)) return repriceResult;
      else if (buyBoxVendorShippingTime < ownVendorShippingTime) {
        repriceResult.repriceDetails.isRepriced = true;
        repriceResult.repriceDetails.explained = RepriceRenewedMessageEnum.BB_SHIPPING;
        const comparablePrice = buyBoxVendor.priceBreaks.find((x: any) => x.minQty == 1)?.unitPrice || 0;
        repriceResult.repriceDetails.newPrice = await subtractPercentage(comparablePrice, getBBShippingValue);
        repriceResult.updateTriggeredBy(buyBoxVendor.vendorName, buyBoxVendor.vendorId, 1);
...
export async function parseBadgeBuyBox(repriceResult: any, net32Result: any, productItem: any): Promise<any> {
  ...
  if (buyBoxVendor && buyBoxVendor.vendorId.toString() == productItem.ownVendorId.toString()) {
    ...
    const comparablePrice = buyBoxVendor.priceBreaks.find((x: any) => x.minQty == 1)?.unitPrice || 0;
    repriceResult.repriceDetails.newPrice = await subtractPercentage(comparablePrice, getBBBadgeValue);
    repriceResult.updateTriggeredBy(buyBoxVendor.vendorName, buyBoxVendor.vendorId, 1);
    return repriceResult;
  }
  return repriceResult;
