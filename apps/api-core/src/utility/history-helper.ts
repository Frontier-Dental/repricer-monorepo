...
export async function Execute(
  ...
  if (
    ...
    for (const $rp of repriceModel.listOfRepriceDetails) {
      const priceHistory = await getHistoricalPrice(
        $rp,
        eligibleList,
        ncFlag,
        mpId,
...
export async function getHistoricalPrice(
  ...
  const suggestedPrice = repriceDetails
    ? repriceDetails.goToPrice
      ? repriceDetails.goToPrice
      : repriceDetails.newPrice
    : 0;
  const rank =
    _.findIndex(
      sortedPayload,
  ...
  return new HistoricalPrice(
    repriceDetails.oldPrice,
    minQty,
    rank,
    repriceDetails.lowestVendor,
    repriceDetails.lowestVendorPrice,
    suggestedPrice,
    repriceDetails.explained,
    sortedPayload,
    eligibleList,
    contextVendor,
    contextCronName,
...
export function getEligibleSortedProducts(
    ...
    if (element.priceBreaks) {
      element.priceBreaks.forEach((p) => {
        if (
          p.minQty === minQty &&
          p.active &&
          isNotShortExpiryProduct(p, element.priceBreaks, minQty) &&
  ...
  if (ncFlag) {
    eligibleList = _.sortBy(tempEligibleList, [
      (prod) =>
        prod.priceBreaks.find((x) => x.minQty === minQty && x.active === true)!
          .unitPrice + GetShippingPrice(prod, minQty),
    ]);
  } else {
    eligibleList = _.sortBy(tempEligibleList, [
      (prod) =>
...
function isNotShortExpiryProduct(
  ...
  const contextPriceBreaks: any = listOfPriceBreaks.find(
...
async function writeFileToSql(data: HistoricalLogs, mpId: string | number) {
  ...
  if (data.historicalPrice && data.historicalPrice.length > 0) {
    ...
    for (const history of data.historicalPrice) {
      const sqlModelHistory = new HistoryModel(
        history,
        mpId,
        data.refTime,
        apiResponseLinkedId,
