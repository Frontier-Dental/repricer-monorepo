...
export async function GetEligibleContextErrorItems(_activeStatus: any, _mpId: any, _contextVendor: any): Promise<ErrorItem[]> {
  ...
  logger.info(`DB_HELPER: Fetching GetEligibleContextErrorItems for mpId: ${_mpId} excluding vendor: ${_contextVendor}`);
  const dbo = await getMongoDb();
  const result = await dbo.collection(applicationConfig.ERROR_ITEM_COLLECTION).find(query).toArray();
  logger.info(`DB_HELPER: Retrieved ${result.length} eligible context error items for mpId: ${_mpId} : ${JSON.stringify(result)}`);
  return result as ErrorItem[];
}
...
export const GetContextErrorItems = async (_activeStatus: any): Promise<any> => {
  ...
  const dbo = await getMongoDb();
  logger.info(`DB_HELPER: Fetching GetContextErrorItems with active status: ${_activeStatus} || Query: ${JSON.stringify(query)}`);
