...
export const GetItemList = async (): Promise<any> => {
  ...
  return dbo.collection(applicationConfig.GET_PRICE_LIST_COLLECTION_NAME).find({ activated: true }).toArray();
};
...
export const UpdateProductAsync = async (payload: any, isPriceUpdated: boolean): Promise<any> => {
  const dbo = await getMongoDb();
  if (isPriceUpdated) {
    return dbo.collection(applicationConfig.GET_PRICE_LIST_COLLECTION_NAME).findOneAndUpdate(
      { mpid: payload.mpid },
      {
        $set: {
...
export const InitCronStatusAsync = async (payload: any): Promise<any> => {
  const dbo = await getMongoDb();
  const result = await dbo.collection(applicationConfig.CRON_STATUS_COLLECTION_NAME).insertOne(payload);
  return result.insertedId.toString();
};

export const UpdateCronStatusAsync = async (payload: any): Promise<any> => {
  const dbo = await getMongoDb();
  return dbo.collection(applicationConfig.CRON_STATUS_COLLECTION_NAME).updateOne(
...
export const GetItemListById = async (_mpId: any): Promise<any> => {
  const dbo = await getMongoDb();
  return dbo.collection(applicationConfig.MANAGED_MONGO_PRODUCT_COLLECTION).findOne({ mpId: _mpId.toString() });
};
...
export const UpsertHistoryDetails = async (payload: any): Promise<any> => {
  const dbo = await getMongoDb();
  const existingItem = await dbo.collection(applicationConfig.HISTORY_DB).findOne({ mpId: payload.mpId });
  if (existingItem) {
...
export const FindErrorItemByIdAndStatus = async (_mpId: any, _status: any): Promise<any> => {
  const query = {
    $and: [
      {
        active: _status,
...
export const GetVendorDetails_ManagedService = async (query: any): Promise<any> => {
  const dbo = await getMongoDb();
  return dbo.collection(applicationConfig.MANAGED_MONGO_COLLECTION).find(query).toArray();
};

export const load_vendor_data = async (payload: any): Promise<any> => {
  const dbo = await getMongoDb();
  return dbo.collection(applicationConfig.MANAGED_MONGO_COLLECTION).insertOne(payload);
};

export const update_vendor_data = async (payload: any): Promise<any> => {
  const dbo = await getMongoDb();
  return dbo.collection(applicationConfig.MANAGED_MONGO_COLLECTION).findOneAndUpdate(
    { mpId: payload.mpId },
    {
      $set: {
