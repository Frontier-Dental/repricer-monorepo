...
export async function getScrappingResponse(_url: string, proxyDetailsResponse: any, seqString: any, retryCount = 0): Promise<any> {
  try {
    ...
    if (response && response.status == 200 && response.data && response.data.results && response.data.results.length > 0) {
      ...
      logger.info(`SCRAPE STARTED : SmartProxy - Web : ContentType : ${responseContentType}`);
      ...
      logger.info(`SCRAPE COMPLETED : SmartProxy - Web : ${_url} || TimeTaken  :  ${parseHrtimeToSeconds(process.hrtime(startTime))} seconds || TaskId : ${taskId} || ${seqString}`);

      return responseData;
    }
  } catch (error: any) {
    logger.info(`SmartProxy - Web Exception : ${error} || URL : ${_url}`);
    logger.error("Error Stack: ", error.stack);
    const errorMessage = getFormattedErrorMesage(error);

    logger.info(`SmartProxy - Web Exception : ${errorMessage} || URL : ${_url}`);
    ...
    if (retryCount < applicationConfig.NO_OF_RETRIES && retryEligible == true) {
      logger.info(`ERROR (WITH RETRY) : ${error} `);
      logger.info(`RETRY ATTEMPT : ${retryCount + 1} at ${new Date()}`);
...
export async function getScrapingBeeResponse(_url: string, proxyDetailsResponse: any, seqString: string | null | undefined, renderJs: boolean, retryCount = 0): Promise<any> {
  try {
    ...
    logger.info(`SCRAPE STARTED : ${scrappingLog} : ${_url} || ${seqString}`);
    var startTime = process.hrtime();
    const options: any = {
    ...
    if (response) {
      logger.info(`SCRAPE COMPLETED : ${scrappingLog} : ${_url} || TimeTaken  :  ${parseHrtimeToSeconds(process.hrtime(startTime))} seconds || ${seqString}`);
    ...
    const errorMessage = getFormattedErrorMesage(error);
    logger.info(`Scraping Bee Exception : ${errorMessage} || URL : ${_url}`);
    await proxySwitchHelper.ExecuteCounter(parseInt(proxyDetailsResponse.proxyProvider));
