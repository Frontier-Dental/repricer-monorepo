...
export async function scrapflyFetchData(url: string, proxyDetailsResponse: ProxyDetailsResponse, seqString: string | null, renderJs: boolean, retryCount = 0): Promise<any> {
  try {
    ...
    return handleRetry(error, retryCount, url, proxyDetailsResponse, seqString, renderJs);
  }
}

export async function scrapflyFetch(urlToScrape: string, proxyHostUrl: string, apiKey: string, renderJs: boolean): Promise<{ responseContent: string; timeTaken: string }> {
...
function validateResponse(data: any, httpStatusCode: number): void {
  if (!data) {
    const error = new Error("Scrapfly response did not return any data") as ScrapflyError;
    error.response = { statusCode: STATUS_CODE_INTERNAL_SERVER_ERROR };
    throw error;
  }

  if (httpStatusCode !== STATUS_CODE_OK) {
    const message = data?.result?.error?.message ? data.result.error.message : `${JSON.stringify(data)}`;
  ...
  if (!data.result || !data.result.content) {
    const error = new Error("Scrapfly response did not return the result.content parameter") as ScrapflyError;
    error.response = { statusCode: STATUS_CODE_INTERNAL_SERVER_ERROR };
    throw error;
  }
}
...
async function handleRetry(error: any, retryCount: number, url: string, proxyDetailsResponse: ProxyDetailsResponse, seqString: string | null, renderJs: boolean): Promise<any> {
  ...
  if (retryCount < applicationConfig.NO_OF_RETRIES && retryEligible) {
    ...
    return await scrapflyFetchData(url, proxyDetailsResponse, seqString, renderJs, retryCount + 1);
  }
