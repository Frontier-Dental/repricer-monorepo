...
import { AlgoExecutionMode, VendorName, VendorIdLookup } from "@repricer-monorepo/shared";
...
export async function Execute(jobId: string, productList: any[], cronInitTime: any, cronSetting: any, isSlowCronRun: boolean = false) {
  ...
  let _contextCronStatus = new CronStatusModel(cronInitTime, 0, productList.length, "In-Progress", cronSetting != null ? cronSetting.CronId : "OVERRIDE_RUN", jobId);
        ...
        if (prod.algo_execution_mode === AlgoExecutionMode.V1_ONLY || prod.algo_execution_mode === AlgoExecutionMode.V1_EXECUTE_V2_DRY || prod.algo_execution_mode === AlgoExecutionMode.V2_EXECUTE_V1_DRY) {
          for (let idx = 0; idx < prioritySequence.length; idx++) {
            ...
            if (proceedNextVendor && isVendorActivated) {
              let repriceResponse = await repriceWrapper(net32resp, prod, cronSetting, isOverrideRun, jobId, prioritySequence, idx, isSlowCronRun);
              eligibleCount++;
              if (repriceResponse) {
...
export async function RepriceErrorItem(details: any, cronInitTime: any, cronSetting: any, _contextVendor: string) {
  let cronLogs: any = {
    time: cronInitTime,
    logs: [] as any[],
        ...
        if (!isPriceUpdatedForVendor) {
          ...
          if (prod.scrapeOn == true && prod.activated == true) {
            ...
            try {
              ...
              const ownVendorData = net32result.data.find((v: any) => v.vendorId && ownVendorList.includes(v.vendorId.toString()));

              if (ownVendorData) {
                const unitPriceBreak = ownVendorData.priceBreaks?.find((pb: { minQty: number }) => pb.minQty === 1);
...
export async function CheckReprice(net32resp: any, prod: any, cronSetting: any, isOverrideRun: boolean, keyGen: string, contextVendor: string) {
  return repriceSingleVendor(net32resp, prod, cronSetting, isOverrideRun, keyGen, contextVendor);
}

export async function RepriceErrorItemV2(productList: any[], cronInitTime: any, keyGen: string) {
  let cronLogs: any = {
    time: cronInitTime,
    keyGen: keyGen,
    logs: [],
  ...
  let _contextCronStatus = new CronStatusModel(cronInitTime, 0, productList.length, "In-Progress", "DUMMY-422-Error", keyGen);
  await initCronStatus(_contextCronStatus);
...
export async function UpdateToMax(cronLogs: any, net32resp: any, prod: any, cronSetting: any, keyGen: string, contextVendor: string) {
  let isPriceUpdated = false;
  ...
  data.prod = prod;
  data.contextVendor = contextVendor;
  const repriceResult = await repriceProductToMax(prod.mpid, data.result, prod, contextVendor);
  if (repriceResult) {
    if (repriceResult.priceUpdateResponse && repriceResult.priceUpdateResponse != null) {
      skipNextVendor = true;
    ...
    try {
      ...
      const ownVendorData = net32resp.data.find((v: { vendorId: { toString: () => string } }) => v.vendorId && v.vendorId.toString() === contextVendorId);
...
async function repriceSingleVendor(net32resp: Net32Response, prod: any, cronSetting: CronSettings, isOverrideRun: boolean, keyGen: string, contextVendor: string, isManualRun = false, isSlowCronRun = false) {
  const cronLogs: any[] = [];
  let isPriceUpdated = false;
  let skipNextVendor = false;
  ...
  // Update History With Proper Message
  if (repriceResult && repriceResult.historyIdentifier && repriceResult.historyIdentifier != null) {
  ...
  try {
    ...
    const ownVendorData = net32resp.data.find((v) => v.vendorId && v.vendorId.toString() === contextVendorId);

    if (ownVendorData) {
      // Get unit price from minQty=1 price break in the Net32 API response
      const unitPriceBreak = ownVendorData.priceBreaks?.find((pb) => pb.minQty === 1);
...
export async function repriceWrapper(net32resp: Net32Response, prod: ProductDetailsListItem, cronSetting: any, isOverrideRun: boolean, keyGen: string, prioritySequence: { name: string; value: string }[], idx: number, isManualRun = false, isSlowCronRun = false) {
  return repriceSingleVendor(net32resp, (prod as any)[prioritySequence[idx].value], cronSetting, isOverrideRun, keyGen, prioritySequence[idx].name, isManualRun, isSlowCronRun);
}
