    ...
    if (sortedPayload && sortedPayload.length > 0) {
      _.first(sortedPayload)!.priceBreaks!.forEach((price: any) => {
        if (price.minQty == 1 && price.active == true) {
          lowestPrice =
            price.unitPrice + GetShippingPrice(_.first(sortedPayload)!);
        }
      });
    }
    ...
    // For Other Vendors
      ...
      if (prodPriceWithMinQty) {
        const lowestPrice =
          ...
          GetShippingPrice(_.first(sortedPayload)!);
        const contextPriceResult = await filterMapper.GetContextPrice(
          parseFloat(lowestPrice as any),
          processOffset,
          floorPrice,
        ...
        if (offsetPrice <= floorPrice) {
          ...
          if (sortedPayload[1] && sortedPayload[1].vendorId == $.VENDOR_ID) {
              ...
              if (nextLowestPrice > contextPrice) {
                if (
                  ...
                  let model = new RepriceModel(
                    ...
                    refProduct,
                    productItem.productName,
                    productItem.maxPrice - standardShippingPrice,
                    true,
                    false,
                    [],
                    RepriceRenewedMessageEnum.PRICE_MAXED,
            ...
            if (
              secondLowestPrice &&
              (secondLowestPrice >= existingPrice ||
                allowCompeteWithNextForFloor === true)
            ) {
              const contextPriceResult = await filterMapper.GetContextPrice(
                parseFloat(secondLowestPrice),
                processOffset,
                floorPrice,
                parseFloat(productItem.percentageDown),
                1,
      ...
      if (comparePrice && (comparePrice as any).Price > 0) {
        ...
        if (
          ...
          repriceModel.repriceDetails!.explained =
            repriceModel.repriceDetails!.explained +
            "_" +
            RepriceRenewedMessageEnum.IGNORED_Q2_VS_Q1_SISTER;
          repriceModel.updateTriggeredBy(
            _.first(q2EligibleListAll).vendorName,
            _.first(q2EligibleListAll).vendorId,
            priceBreak.minQty,
          );
          return repriceModel;
        }
        ...
        if (
          ...
          repriceModel.repriceDetails!.isRepriced = true;
          repriceModel.repriceDetails!.explained =
            RepriceRenewedMessageEnum.REPRICE_Q2_VS_Q1;
          if ((comparePrice as any).Type === "Q2") {
            repriceModel.updateTriggeredBy(
              _.first(q2EligibleList).vendorName,
              _.first(q2EligibleList).vendorId,
              priceBreak.minQty,
            );
          } else {
            repriceModel.updateTriggeredBy(
              _.first(eligibleList).vendorName,
              _.first(eligibleList).vendorId,
              priceBreak.minQty,
            );
          }
        } else {
          repriceModel.repriceDetails!.explained =
            repriceModel.repriceDetails!.explained +
            "_" +
            RepriceRenewedMessageEnum.IGNORED_Q2_VS_Q1;
        }
      }
      if (repriceModel.repriceDetails!.isRepriced === true) {
        return repriceModel;
    ...
    let sortedPayload = _.sortBy(tempEligibleList, [
      (prod) => {
        return (
          (
            _.find(prod.priceBreaks!, (x) => {
              if (x.minQty == priceBreak.minQty && x.active == true)
                return true;
            }) as any
            ...
            if (contextPrice.toFixed(2) !== existingPrice.toFixed(2)) {
              if (contextPrice - standardShippingPrice >= floorPrice) {
                repriceModel.repriceDetails!.newPrice = await getSetPrice(
                  ...
                  productItem.freeShippingThreshold,
                  priceBreak.minQty,
    ...
    // For Other Vendors
      ...
      if (
        ...
        const goToPriceCalc = await getSetPrice(
          ...
          refProduct.standardShippingPrice,
          refProduct.freeShippingThreshold,
          priceBreak.minQty,
        );
          ...
          if (priceBreak.minQty != 1 && existingPrice != 0) {
            ...
            const nextLowestPrice =
              (
                _.find(sortedPayload[nextIndex].priceBreaks, (price) => {
                  if (
                    price.minQty == priceBreak.minQty &&
                    price.active == true
                  ) {
                    return true;
                  }
            ...
            if (secondLowestPrice) {
              ...
              if (
                ...
                const contextPriceResult = await filterMapper.GetContextPrice(
                  parseFloat(nextLowestPriceForNRank),
                  processOffset,
                  floorPrice,
                  parseFloat(productItem.percentageDown),
                  priceBreak.minQty,
