...
import { RepriceMessageEnum } from "../model/repriceMessage";
import { RepriceRenewedMessageEnum } from "../model/repriceRenewedMessage";
import * as filterMapper from "./filterMapper";
    ...
    case 0: //Only Up
      if ($eval.listOfRepriceDetails && $eval.listOfRepriceDetails.length > 0) {
        ...
        if ($eval.repriceDetails.newPrice != "N/A") {
          const calculatedOldPrice =
            isNcNeeded == false
              ? parseFloat($eval.repriceDetails.oldPrice as unknown as string)
              : getNcCalculatedPrice($eval.repriceDetails, net32Details);
          if (
            parseFloat($eval.repriceDetails.newPrice as unknown as string) <
            calculatedOldPrice
          ) {
            $eval.repriceDetails.goToPrice = $eval.repriceDetails
              .newPrice as unknown as string;
...
export function ApplyMultiPriceBreakRule(repriceResult: RepriceModel) {
  repriceResult.listOfRepriceDetails = _.sortBy(
  ...
  if ($eval.listOfRepriceDetails && $eval.listOfRepriceDetails.length > 0) {
    ...
    let _eval_ = [];
    for (
      let idx = repriceResult.listOfRepriceDetails.length - 1;
      idx >= 0;
      idx--
    ) {
      if (repriceResult.listOfRepriceDetails[idx].minQty === 1) {
        ...
        const sourcePrice =
          repriceResult.listOfRepriceDetails[idx].newPrice !== "N/A"
            ? parseFloat(
                repriceResult.listOfRepriceDetails[idx]
                  .newPrice as unknown as string,
              )
        ...
        for (let k = 0; k < idx; k++) {
          const comparablePrice =
            repriceResult.listOfRepriceDetails[k].newPrice !== "N/A"
              ? parseFloat(
                  repriceResult.listOfRepriceDetails[k]
                    .newPrice as unknown as string,
                )
        ...
        if (success === true) {
          if (repriceResult.listOfRepriceDetails[idx].oldPrice !== 0) {
            ...
            repriceResult.listOfRepriceDetails[idx].oldPrice === 0 &&
            repriceResult.listOfRepriceDetails[idx].newPrice !== "N/A"
          ) {
            _eval_.push(repriceResult.listOfRepriceDetails[idx]);
          }
        } else if (repriceResult.listOfRepriceDetails[idx].oldPrice !== 0) {
          let dummyPricePoint = _.cloneDeep(
            repriceResult.listOfRepriceDetails[idx],
          );
          if (
            dummyPricePoint.explained ===
            ...
            dummyPricePoint.explained =
              dummyPricePoint.explained +
              "_" +
              RepriceRenewedMessageEnum.IGNORE_LOWEST_PRICE_BREAK;
          }
...
export function ApplySuppressPriceBreakRule(
  repriceResult: RepriceModel,
  minQty: number,
  isOverrideEnabled: boolean,
) {
  let $eval = _.cloneDeep(repriceResult);
  const isOneQtyChanged = validateQtyReprice(
    $eval.listOfRepriceDetails,
    minQty,
    isOverrideEnabled,
  );
  ...
  if ($eval.listOfRepriceDetails && $eval.listOfRepriceDetails.length > 0) {
    ...
    _.remove(
      $eval.listOfRepriceDetails,
      (rp) => rp.newPrice == "N/A" && rp.oldPrice == 0,
    );
  }
  return $eval;
}
...
export function ApplyPercentagePriceRule(
  repriceResult: RepriceModel,
  percentage: number,
) {
  let $eval = repriceResult;
  if ($eval.listOfRepriceDetails && $eval.listOfRepriceDetails.length > 0) {
    ...
    if (
      ...
      if (!isValid) {
        ...
        $eval.repriceDetails.isRepriced = false;
        $eval.repriceDetails.explained =
          RepriceMessageEnum.IGNORED_PERCENTAGE_CHECK;
      }
    }
  }
  return $eval;
...
export function ApplyDeactivateQPriceBreakRule(
  repriceResult: RepriceModel,
  ...
  if ($eval.listOfRepriceDetails && $eval.listOfRepriceDetails.length > 0) {
    ...
    for (let $ of $eval.listOfRepriceDetails) {
      if ($.minQty != 1) {
        const isContextQtyDeactivated = validateQtyDeactivation(
          $eval.listOfRepriceDetails,
        ...
        if (
          ...
          $.explained =
            $.explained +
            RepriceRenewedMessageEnum.IGNORED_ABORT_Q_DEACTIVATION;
        }
      }
    }
  }
...
export function ApplyBuyBoxRule(
  repriceResult: RepriceModel,
  ...
  if ($eval.listOfRepriceDetails && $eval.listOfRepriceDetails.length > 0) {
    $eval.listOfRepriceDetails.forEach(($) => {
      if (
        ...
        if (
          firstItem &&
          _.includes(contextVendorIds, (firstItem as any).vendorId.toString())
        ) {
          $.goToPrice = $.newPrice;
          $.newPrice = "N/A";
          $.isRepriced = false;
    ...
    if (
      $eval.repriceDetails.oldPrice != 0 &&
      $eval.repriceDetails.newPrice != "N/A" &&
      parseFloat($eval.repriceDetails.newPrice as unknown as string) <
        parseFloat($eval.repriceDetails.oldPrice as unknown as string)
    ) {
      ...
      if (
        ...
        $eval.repriceDetails.isRepriced = false;
        $eval.repriceDetails.explained =
          RepriceRenewedMessageEnum.IGNORE_BUY_BOX;
      }
    }
  }
  return $eval;
...
export function ApplyFloorCheckRule(
  repriceResult: RepriceModel,
  floorPrice: number,
) {
  let $eval = repriceResult;
  if ($eval.listOfRepriceDetails && $eval.listOfRepriceDetails.length > 0) {
    ...
    $eval.repriceDetails &&
    ($eval.repriceDetails.newPrice as unknown as number) <= floorPrice
  ) {
    $eval.repriceDetails.goToPrice = $eval.repriceDetails
      .newPrice as unknown as string;
    $eval.repriceDetails.newPrice = "N/A";
...
export function ApplyKeepPositionLogic(
  repriceResult: RepriceModel,
  net32Result: Net32Product[],
  ownVendorId: string,
) {
  ...
  if ($eval.listOfRepriceDetails && $eval.listOfRepriceDetails.length > 0) {
    ...
    if (
      ...
      if (evalVendorIndex > ownVendorIndex) {
        ...
        $eval.repriceDetails.isRepriced = false;
        $eval.repriceDetails.explained =
          RepriceRenewedMessageEnum.IGNORE_KEEP_POSITION;
      }
    }
  }
  return $eval;
...
export function AppendNewPriceBreakActivation(
  repriceResult: RepriceModel,
): RepriceModel {
  if (
    repriceResult.listOfRepriceDetails &&
    ...
    if (
      repriceResult.repriceDetails.oldPrice == 0 &&
      repriceResult.repriceDetails.newPrice != "N/A" &&
      parseFloat(repriceResult.repriceDetails.newPrice as unknown as string) >
        parseFloat(repriceResult.repriceDetails.oldPrice as unknown as string)
    ) {
      if (repriceResult.repriceDetails.explained!.indexOf("#UP") >= 0) {
        repriceResult.repriceDetails.explained =
          repriceResult.repriceDetails.explained!.replace("#UP", "#NEW");
      } else {
...
export async function ApplyRepriceDownBadgeCheckRule(
  ...
  const listOfAuthorizedVendors = await filterMapper.FilterBasedOnParams(
    net32Result,
    tempProductItem,
    "BADGE_INDICATOR",
  );
  if (listOfAuthorizedVendors != null && listOfAuthorizedVendors.length > 0) {
    ...
    if (
      _.includes(
        excludedVendors,
        _.first(sortedAuthVendors).vendorId.toString(),
      )
    ) {
      return $eval;
    }
    _.remove(
      sortedAuthVendors,
      (data: any) => data.vendorId == productItem.ownVendorId,
    );
    _.remove(sortedAuthVendors, (data: any) =>
      _.includes(excludedVendors, data.vendorId.toString()),
    );
    if (sortedAuthVendors.length > 0) {
      ...
      _.forEach(_.first(sortedAuthVendors).priceBreaks, (pb: any) => {
        if (pb.minQty == 1 && pb.active == true) {
          lowestUnitPrice =
            pb.unitPrice + GetShippingPrice(_.first(sortedAuthVendors));
        }
      });
      ...
      if (calculatedSuggestedPrice <= effectivePrice) {
        ...
        if (effectivePrice <= floorPriceOfProduct) {
          ...
          if (effectivePrice > floorPriceOfProduct) {
            if (
              $eval.listOfRepriceDetails &&
              $eval.listOfRepriceDetails.length > 0
            ) {
              $eval.listOfRepriceDetails.forEach(($: any) => {
                if ($.minQty == 1) {
                  $.goToPrice = null;
          ...
          effectivePrice > floorPriceOfProduct &&
          effectivePrice != existingPrice
        ) {
          if (
            $eval.listOfRepriceDetails &&
            $eval.listOfRepriceDetails.length > 0
...
export async function ApplySisterComparisonCheck(
  ...
  const sisterVendorResults = net32Result.filter(
    (x) =>
      _.includes(sisterVendors, x.vendorId.toString()) && x.inStock == true,
  );
  if ($eval.listOfRepriceDetails && $eval.listOfRepriceDetails.length > 0) {
    for (let $ of $eval.listOfRepriceDetails) {
      if ($.newPrice != "N/A") {
        const ownVendorSuggestedPrice = parseFloat(
          $.newPrice as unknown as string,
        );
            ...
            if (item.priceBreaks && item.priceBreaks.length > 0) {
              for (const pb of item.priceBreaks) {
                if (
                  pb.minQty == $.minQty &&
                  pb.active == true &&
                  pb.unitPrice == ownVendorSuggestedPrice
                ) {
              ...
              const sisterVendorWithSuggestedPrice = sisterVendorResults.find(
                (x) => {
                  if (x.priceBreaks && x.priceBreaks.length > 0) {
                    const priceBreak = x.priceBreaks.find(
                      (pb) =>
                        pb.minQty == 1 &&
                        pb.active == true &&
                        (pb.unitPrice as unknown as string) == suggestedPrice,
                    );
    ...
    const ownVendorSuggestedPrice = parseFloat(
      $eval.repriceDetails.newPrice as unknown as string,
    );
    if (sisterVendorResults && sisterVendorResults.length > 0) {
      ...
      if (sisterVendorSimilarPrice.length > 0) {
        ...
        $eval.repriceDetails.explained =
          $eval.repriceDetails.explained + " #SISTERSAMEPRICE";
      }
    }
  }
  return $eval;
...
function validateQtyReprice(
  ...
  if (
    contextPriceBreak &&
    contextPriceBreak.newPrice != "N/A" &&
    contextPriceBreak.oldPrice != parseFloat(contextPriceBreak.newPrice)
  ) {
    return true;
  }
  return false;
...
function executePercentageCheck(
  result: any,
  expectedPercentage: number,
): boolean {
  const percentageIncrease =
    ((parseFloat(result.newPrice) - result.oldPrice) / result.oldPrice) * 100;
  return percentageIncrease >= expectedPercentage;
}

function validateQtyDeactivation(
  listOfRepriceDetails: any[],
  minQty: number,
): boolean {
  const contextPriceBreak = listOfRepriceDetails.find(
  ...
  if (
    contextPriceBreak &&
    contextPriceBreak.newPrice != "N/A" &&
    parseFloat(contextPriceBreak.newPrice) == 0
  ) {
    return true;
  }
  return false;
...
function getNcCalculatedPrice(repriceDetails: any, net32Details: any): number {
  if (repriceDetails && repriceDetails.minQty) {
    ...
    const existingUnitPrice = net32Details.priceBreaks.find(
      (x: any) => x.minQty == repriceDetails.minQty,
    ).unitPrice;
    ...
    const existingUnitPrice = (
      _.first(net32Details.priceBreaks) as { unitPrice: number }
    ).unitPrice;
    const thresholdPrice = net32Details.freeShippingThreshold;
    if (existingUnitPrice < thresholdPrice) {
      const shippingCharges = parseFloat(net32Details.standardShipping);
...
function getSuggestedPriceForMinQty(
  repricerResult: any,
  minQty: number,
): number {
  ...
  if (
    repricerResult &&
    repricerResult.listOfRepriceDetails &&
    repricerResult.listOfRepriceDetails.length > 0
  ) {
    const minQtyResult = repricerResult.listOfRepriceDetails.find(
      (x: any) => x.minQty == minQty,
    );
    suggestedPrice =
      minQtyResult && minQtyResult.newPrice != "N/A"
        ? parseFloat(minQtyResult.newPrice)
        : suggestedPrice;
  } else if (
    repricerResult &&
    repricerResult.repriceDetails &&
    repricerResult.repriceDetails.newPrice != "N/A"
  ) {
  ...
  if (item != null && item.priceBreaks && item.priceBreaks.length > 0) {
    const thresholdPrice =
      item.freeShippingThreshold != null && item.freeShippingThreshold >= 0
        ...
        : 999999;
    const unitPrice = item.priceBreaks.find(
      (x: any) => x.minQty == 1,
    ).unitPrice;
