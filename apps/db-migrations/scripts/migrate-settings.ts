...
import { AlgoBadgeIndicator, AlgoPriceDirection, AlgoPriceStrategy } from "@repricer-monorepo/shared";
...
function transformVendorSettings(vendorSettings: any[], vendorConfig: VendorConfig) {
...
async function performBatchUpsert(db: any, transformedSettings: any[], vendorConfig: VendorConfig): Promise<{ insertedCount: number; updatedCount: number }> {
  let insertedCount = 0;
  let updatedCount = 0;
  ...
  try {
    ...
    const existingMap = new Map(existingRecords.map((r) => [`${r.mp_id}-${r.vendor_id}`, true]));
    ...
    if (toInsert.length > 0) {
      ...
      console.log(`✅ Batch inserted ${toInsert.length} new settings for ${vendorConfig.vendorName}`);
    }
    ...
    if (toUpdate.length > 0) {
      ...
      console.log(`🔄 Batch updated ${toUpdate.length} existing settings for ${vendorConfig.vendorName}`);
    }
  } catch (error) {
...
async function migrateVendorSettings(vendorConfig: VendorConfig) {
  ...
  try {
    console.log(`🔄 Starting migration of ${vendorConfig.vendorName} settings to v2_algo_settings...`);
    ...
    if (!v2SettingsExists) {
      console.log("❌ v2_algo_settings table does not exist. Please run migrations first.");
      return;
    }
    ...
    console.log(`📊 Found ${vendorSettings.length} ${vendorConfig.vendorName} settings to migrate`);

    if (vendorSettings.length === 0) {
      console.log(`ℹ️  No ${vendorConfig.vendorName} settings found to migrate`);
      return;
    }
    ...
    const { insertedCount, updatedCount } = await performBatchUpsert(db, transformedSettings, vendorConfig);

    console.log(`✅ Successfully migrated ${transformedSettings.length} ${vendorConfig.vendorName} settings to v2_algo_settings`);
    console.log(`📝 Inserted: ${insertedCount}, Updated: ${updatedCount}`);
    console.log(`📋 Vendor ID set to: ${vendorConfig.vendorId} (${vendorConfig.vendorName})`);

    // Verify the migration
    const finalCount = await db("v2_algo_settings").where("vendor_id", vendorConfig.vendorId).count("* as count").first();

    console.log(`📊 Total ${vendorConfig.vendorName} settings in v2_algo_settings: ${finalCount ? finalCount.count : 0}`);

    return { insertedCount, updatedCount } as MigrationResult;
  } catch (error) {
    console.error(`❌ Error during ${vendorConfig.vendorName} migration:`, error);
    throw error;
  } finally {
    if (db) {
      await db.destroy();
...
async function migrateAllVendors() {
  ...
  console.log(`📋 Vendors to migrate: ${vendorConfigs.map((v) => v.vendorName).join(", ")}`);
  ...
  for (const vendorConfig of vendorConfigs) {
    try {
      ...
      console.log(`✅ ${vendorConfig.vendorName}: ${inserted} inserted, ${updated} updated`);
  ...
  console.log(`✅ Successful migrations: ${successCount}/${vendorConfigs.length}`);
...
async function syncSingleMpId(mpId: number) {
  ...
  console.log(`📋 Vendors to sync: ${vendorConfigs.map((v) => v.vendorName).join(", ")}`);
  ...
  for (const vendorConfig of vendorConfigs) {
    try {
      console.log(`\n${"=".repeat(60)}`);
      console.log(`🔄 Starting ${vendorConfig.vendorName} sync for MP ID ${mpId}...`);

      const result = await syncVendorSettingsForMpId(vendorConfig, mpId);

      console.log(`✅ Completed ${vendorConfig.vendorName} sync for MP ID ${mpId}`);
      ...
      console.error(`❌ Failed to sync ${vendorConfig.vendorName} for MP ID ${mpId}:`, error);
      errorCount++;
    }
  }
...
async function syncVendorSettingsForMpId(vendorConfig: VendorConfig, mpId: number) {
  const db = createKnexInstance();

  try {
    console.log(`🔄 Starting sync of ${vendorConfig.vendorName} settings for MP ID ${mpId}...`);
    ...
    const vendorSettings = await db(vendorConfig.tableName).where("MpId", mpId).select("*");

    console.log(`📊 Found ${vendorSettings.length} ${vendorConfig.vendorName} settings for MP ID ${mpId}`);

    if (vendorSettings.length === 0) {
      console.log(`ℹ️  No ${vendorConfig.vendorName} settings found for MP ID ${mpId}`);
      return { insertedCount: 0, updatedCount: 0 };
    }
    ...
    console.log(`✅ Successfully synced ${vendorConfig.vendorName} settings for MP ID ${mpId}`);
    console.log(`📝 Inserted: ${insertedCount}, Updated: ${updatedCount}`);

    return { insertedCount, updatedCount } as MigrationResult;
  } catch (error) {
    console.error(`❌ Error during ${vendorConfig.vendorName} sync for MP ID ${mpId}:`, error);
    throw error;
  } finally {
    if (db) {
      await db.destroy();
