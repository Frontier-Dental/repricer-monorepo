...
DROP PROCEDURE IF EXISTS sp_GetMasterDataWithCount;

DELIMITER / /
CREATE PROCEDURE sp_GetMasterDataWithCount (IN recordCount int) BEGIN DECLARE time_value_1 varchar(25);

DECLARE time_value_2 varchar(25);

DECLARE time_value_3 varchar(25);

SET
  time_value_1 = concat(DATE_FORMAT(CURDATE(), '%d-%m-%Y'), '%'),
  time_value_2 = CONCAT(
...
Insert into
  tmp_tbl_runInfo (RunId, Mpid, CronName) (
    ...
    where
      ...
      and (
        ri.RunEndTime like time_value_1
        or ri.RunEndTime like time_value_2
        or ri.RunEndTime like time_value_3
      )
    order by
      ri.Id desc
  );
