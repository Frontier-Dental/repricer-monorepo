...
const ResetSlowCronUpdate = asyncHandler(async (req, res) => {
  const query = {
    $or: [
...
const RefillParentCronDetails = asyncHandler(async (req, res) => {
  ...
  if (payload && payload.length > 0) {
    for (const mpId of payload) {
      let product = _.first(await mongoMiddleware.FindProductById(mpId));
      const contextCronDetails = await getContextCronDetails(
        product,
...
const CorrectSlowCronDetails = asyncHandler(async (req, res) => {
...
const MapVendorToRoot = asyncHandler(async (req, res) => {
...
const GetFloorBelowProducts = asyncHandler(async (req, res) => {
...
const DeleteProdHistory = asyncHandler(async (req, res) => {
  try {
    const { startDate, endDate } = req.body;
    ...
    console.log(`Exception while DeleteProdHistory : ERROR : ${ex}`);
    return res.json(
      `DeleteHistory failed from Live for StartDate : ${startDate} | EndDate : ${endDate} with error ${ex}`,
    );
  }
});
...
async function getContextCronDetails(product, cronSettingsResponse) {
  let data = {};
...
async function startDeletingHistoryFromProduction(_startDate, _endDate) {
...
module.exports = {
  ResetSlowCronUpdate,
  RefillParentCronDetails,
  ScrapeProduct,
  CorrectSlowCronDetails,
  MapVendorToRoot,
  GetFloorBelowProducts,
  DeleteHistory,
  DeleteProdHistory,
};
