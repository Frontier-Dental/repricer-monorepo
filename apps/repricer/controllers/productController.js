...
const scheduleConstants = require("../models/scheduleConstants");
const mongoMiddleware = require("../middleware/mongoMiddleware");
const moment = require("moment");
_constants = new scheduleConstants();
...
const handlingTimeGroupResx = require("../resources/HandlingTimeFilterMapping.json");

const getMasterItemController = asyncHandler(async (req, res) => {
  let query = {};
  let tags = "";
...
const addMasterItemController = asyncHandler(async (req, res) => {
  const cronSettings = await mongoMiddleware.GetCronSettingsList();
...
const editMasterItemController = asyncHandler(async (req, res) => {
  ...
  item.cronName = item.cronId
    ? cronSettings.find((x) => x.CronId == item.cronId).CronName
    : "";
...
const deleteMasterItemController = asyncHandler(async (req, res) => {
...
const addMasterItemToDatabase = asyncHandler(async (req, res) => {
  let data = {};
...
const updateMasterItemController = asyncHandler(async (req, res) => {
  let data = {};
  data.cronId = req.body.cronGroup;
  data.channelName = req.body.channel_name;
  data.productName = req.body.product_name;
...
const excelDownload = async (req, res) => {
  let ItemCollection = await Item.find();
  const cronSettings = await mongoMiddleware.GetCronSettingsList();
  ItemCollection.forEach((_) => {
    if (_.cronId) {
      _.cronName = cronSettings.find((x) => x.CronId == _.cronId).CronName;
    }
    if (_.tags) {
...
const runAllCron = asyncHandler(async (req, res) => {
  const cronSettings = await mongoMiddleware.GetCronSettingsList();
  if (cronSettings && cronSettings.length > 0) {
    for (const cron of cronSettings) {
      if (cron.IsHidden & (cron.IsHidden == true)) {
        ...
        await mongoMiddleware.ToggleCronStatus(cron.CronId, true, req);
      }
    }
  }
...
const runManualCron = asyncHandler(async (req, res) => {
...
const resetCron = asyncHandler(async (req, res) => {
...
const addExcelData = asyncHandler(async (req, res) => {
  let input = req.body;
  ...
  try {
    ...
    for (let k = 0; k < parseInt(input.count); k++) {
      ...
      if (JSON.parse(process.env.USE_MYSQL) == true) {
        let $item = {
          ...
          cronName: row[13],
          cronId: row[13]
            ? combinedArray.find((x) => x.CronName == row[13].trim()).CronId
            : "",
          scrapeOn:
            row[14] != null && row[14] != "" ? JSON.parse(row[14]) : true,
          allowReprice:
          ...
          scrapeOnlyCronId: row[19]
            ? scrapeOnlyCrons.find((x) => x.CronName == row[19].trim()).CronId
            : "",
          ...
          slowCronId: row[41]
            ? slowCrons.find((x) => x.CronName == row[41].trim()).CronId
            : "", //Get Data
          isSlowActivated:
          ...
          handlingTimeFilter: row[48]
            ? handlingTimeGroupResx.find((x) => x.value == row[48].trim()).key
            : "ALL",
        ...
        let $item = {
          ...
          cronId: row[30]
            ? combinedArray.find((x) => x.CronName == row[30].trim()).CronId
            : "",
          cronName: row[30],
    ...
    let uniqueProductIds = await GetUniqueProductIds(items);
    for (const pId of uniqueProductIds) {
      let itemData = {};
      itemData.mpId = pId;
      itemData.tradentDetails = items.find(
        (x) => x.channelName.toUpperCase() == "TRADENT" && x.mpid == pId,
      );
      if (
      ...
      itemData.frontierDetails = items.find(
        (x) => x.channelName.toUpperCase() == "FRONTIER" && x.mpid == pId,
      );
      if (
      ...
      itemData.mvpDetails = items.find(
        (x) => x.channelName.toUpperCase() == "MVP" && x.mpid == pId,
      );
      if (
        itemData.mvpDetails &&
        itemData.mvpDetails.executionPriority == null
      ) {
        itemData.mvpDetails.executionPriority = 3;
      }
      itemData.topDentDetails = items.find(
        (x) => x.channelName.toUpperCase() == "TOPDENT" && x.mpid == pId,
      );
      if (
      ...
      itemData.firstDentDetails = items.find(
        (x) => x.channelName.toUpperCase() == "FIRSTDENT" && x.mpid == pId,
      );
      if (
...
async function GetUniqueProductIds(collatedList) {
...
const stopAllCron = asyncHandler(async (req, res) => {
  var cronStopResponse = await httpMiddleware.stopAllCron();
    ...
    if (cronSettings && cronSettings.length > 0) {
      for (const cron of cronSettings) {
        if (cron.IsHidden & (cron.IsHidden == true)) {
          ...
          await mongoMiddleware.ToggleCronStatus(cron.CronId, false, req);
        }
      }
    }
    return res.json({
...
async function getProductList(dict) {
  let resultProducts = [];
  const keys = Object.keys(dict);
  if (keys) {
    for (const key of keys) {
      resultProducts.push(dict[key]);
    }
  }
  return resultProducts;
}

async function createDummyCronLog(item, updatedResponse) {
  let result = {};
...
async function getSecretKey(_cronId) {
  const cronDetails = await mongoMiddleware.GetCronSettingsList();
  const contextCron = cronDetails.find((x) => x.CronId == _cronId);
  return contextCron ? contextCron.SecretKey : null;
}

async function getPriceUpdatedField(updatedResponse) {
  return (
    updatedResponse &&
    updatedResponse.message &&
...
function parseBadgeIndicator(stringValue, evalType) {
...
const start_override = asyncHandler(async (req, res) => {
...
module.exports = {
  ...
  addMasterItemController,
  updateMasterItemController,
  addMasterItemToDatabase,
  editMasterItemController,
  deleteMasterItemController,
  excelDownload,
  runAllCron,
  runManualCron,
  resetCron,
