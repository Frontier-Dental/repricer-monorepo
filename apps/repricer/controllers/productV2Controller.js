...
const productHelper = require("../middleware/productHelper");
const mapper = require("../middleware/mapperHelper");
const badgeResx = require("../resources/badgeIndicatorMapping.json");
const ProductModel = require("../models/product");
const excelJs = require("exceljs");
const moment = require("moment");
const mySqlHelper = require("../middleware/mySQLMiddleware");
...
const collateProducts = asyncHandler(async (req, res) => {
...
const editItemView = asyncHandler(async (req, res) => {
...
const collateProductsForId = asyncHandler(async (req, res) => {
...
const addItems = asyncHandler(async (req, res) => {
  ...
  productDetails.slowCrons = slowCronSettings.filter((x) => x.IsHidden != true);
...
const addItemToDatabase = asyncHandler(async (req, res) => {
  var details = req.body;
  let productDetails = {};
  const scrapeOnlyCronSettings = await mongoMiddleware.GetScrapeCrons();
  if (details.tradentDetails && details.tradentDetails != null) {
    ...
    productDetails.tradentDetails.scrapeOnlyCronName =
      scrapeOnlyCronSettings.find(
        (x) => (x.CronId = details.scrapeOnlyCron),
      ).CronName;
...
const runManualCron = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    console.log(`Exception while Manual Run error : ${exception}`);
    return res.json({
      status: false,
      message: `Manual Scrape Done Successfully!`,
    });
...
const syncProductDetails = asyncHandler(async (req, res) => {
  try {
    ...
    if (
      ...
      let productDetails = _.first(getProdResponse.data.message);
...
const runManualSyncOfProducts = asyncHandler(async (req, res) => {
...
const removeFrom422 = asyncHandler(async (req, res) => {
  const selectedProducts = Array.isArray(req.body.mpIds)
    ? req.body.mpIds
    : [req.body.mpIds];
  try {
    ...
    console.log(`Exception while removeFrom422 error : ${exception}`);
    return res.json({
      status: false,
      message: `Failed to remove from 422!!`,
    });
  }
});

const removeFrom422ForAll = asyncHandler(async (req, res) => {
  try {
...
const toggleDataScrape = asyncHandler(async (req, res) => {
  if (req.body.mpid) {
...
const saveRootDetails = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const scrapeOnlyCronSettingsDetails =
      await mongoMiddleware.GetScrapeCrons();
    const linkedRegularCronName = regularCronSettingsDetails.find(
      (x) => x.CronId == rootDetailsForPayload.cronGroup,
    ).CronName;
    const scrapeOnlyCronName = scrapeOnlyCronSettingsDetails.find(
      (x) => x.CronId == rootDetailsForPayload.scrapeOnlyCron,
    ).CronName;
    ...
    console.log(
      `Exception while saving root details for ${mpid} || ERROR : ${exception}`,
    );
    return res.json({
      status: false,
      message: `${mpidTrimmed} ${exception.message}.`,
    });
  }
});

const activateProductForAll = asyncHandler(async (req, res) => {
  if (req.body.mpid) {
...
const deActivateProductForAll = asyncHandler(async (req, res) => {
  if (req.body.mpid) {
    removeItems = await mySqlHelper.ChangeProductActivation(
      parseInt(req.body.mpid.trim()),
      false,
...
const saveBranches = asyncHandler(async (req, res) => {
  try {
    const {
      mpid,
      tradentDetails,
    ...
    if (frontierDetails && Object.keys(frontierDetails).length > 0) {
      ...
      for (const key in frontierDetails) {
        if (
          frontierDetails[key] !== null &&
          frontierDetails[key] !== undefined
        ) {
...
const updateToMax = asyncHandler(async (req, res) => {
  const selectedProducts = Array.isArray(req.body.mpIds)
    ? req.body.mpIds
    : [req.body.mpIds];
  let failedIds = [];
  try {
    ...
    console.log(`Exception while Update to max error : ${exception}`);
    return res.json({
      status: false,
      message: `Update to Max failed!`,
    });
  }
});
...
async function spliceResult(arrayResult, pageNo, pageSize) {
