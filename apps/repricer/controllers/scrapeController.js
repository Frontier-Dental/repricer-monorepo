...
const GetScrapeCron = asyncHandler(async (req, res) => {
...
const UpdateScrapeCronExp = asyncHandler(async (req, res) => {
  ...
  for (const en in normalizedRequestPayload.cron_id_hdn) {
    ...
    if (
      ...
      updatedList.push(scrapeSettingPayload);
      listOfUpdatedCronKey.push(
        cronMapping.find((c) => c.cronId == scrapeSettingPayload.CronId)
          .cronVariable,
      );
    }
  }
...
const GetLatestPriceInfo = asyncHandler(async (req, res) => {
  ...
  if (productDetails && productDetails.length > 0) {
    ...
    const mpId = contextProduct.mpId;
...
const GetScrapeProducts = asyncHandler(async (req, res) => {
  let scrapeCronDetails = await mongoMiddleware.GetScrapeCrons();
  let pgNo = 0;
  ...
  try {
    ...
    for (let k = 0; k < parseInt(input.count) - 1; k++) {
      ...
      let $item = {
        ...
        linkedCronId: row[3]
          ? scrapeCron.find((x) => x.CronName == row[3].trim()).CronId
          : "",
    ...
    for (const item of items) {
      if (item.mpId != "") {
        console.log(`EXCEL-IMPORT : Inserting ${item.mpId} to mySQL`);
...
const deleteItem = asyncHandler(async (req, res) => {
  ...
  const item = await sqlMiddleware.DeleteScrapeProductById(parseInt(id));
  if (item) {
    return res.json({
      status: true,
...
async function normalizePayload(requestedPayload) {
  const normalizeToArray = (item) => (Array.isArray(item) ? item : [item]);
