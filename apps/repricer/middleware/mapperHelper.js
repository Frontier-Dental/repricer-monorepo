...
const itemMapper = require("../resources/itemMapper.json");
const badgeResx = require("../resources/badgeIndicatorMapping.json");
const FailedReport = require("../models/failedReport");
const mongoUtility = require("../middleware/mongoMiddleware");
const moment = require("moment");
const { exceptions } = require("winston");
const LatestScrapeInfo = require("../models/ApiResponse/LatestScrapeInfo");
const mySqlUtility = require("../middleware/mySQLMiddleware");
const MySqlProduct = require("../models/SqlModels/MySQLProduct");
const SessionHelper = require("../Utility/SessionHelper");

module.exports.MapV2 = async (productDetails) => {
  let $eval = [];
...
module.exports.MapBadgeIndicator = async (product) => {
...
module.exports.MapCronName = async (prod, cronSettings) => {
  if (prod.tradentDetails && prod.tradentDetails.cronId) {
    prod.tradentDetails.cronName = cronSettings.find(
      (x) => x.CronId == prod.tradentDetails.cronId,
    )
      ? cronSettings.find((x) => x.CronId == prod.tradentDetails.cronId)
          .CronName
      : "N/A";
  }
  if (prod.frontierDetails && prod.frontierDetails.cronId) {
    prod.frontierDetails.cronName = cronSettings.find(
      (x) => x.CronId == prod.frontierDetails.cronId,
    )
      ? cronSettings.find((x) => x.CronId == prod.frontierDetails.cronId)
          .CronName
      : "N/A";
  }
  if (prod.mvpDetails && prod.mvpDetails.cronId) {
    prod.mvpDetails.cronName = cronSettings.find(
      (x) => x.CronId == prod.mvpDetails.cronId,
    )
      ? cronSettings.find((x) => x.CronId == prod.mvpDetails.cronId).CronName
      : "N/A";
  }
  return prod;
};

module.exports.MapUserResponse = async (
  productDetails,
  updateDetails,
...
module.exports.MapFormData = async (
  productDetails,
  formData,
  formVendorData,
) => {
...
module.exports.AlignCronName = async (productList) => {
  try {
    let alignedProduct = [];
    for (var product of productList) {
      if (!_.includes(alignedProduct, product.mpid)) {
        ...
        if (listOfRelatedProducts && listOfRelatedProducts.length > 0) {
          const contextCronName = _.last(listOfRelatedProducts).cronName;
          const contextCronId = _.last(listOfRelatedProducts).cronId;
          _.forEach(listOfRelatedProducts, (p) => {
            p.cronName = contextCronName;
            p.cronId = contextCronId;
          });
        }
        alignedProduct.push(product.mpid);
      }
    }
  } catch (exception) {
...
module.exports.MapScrapedFailedResults = async (cronLogs) => {
  let result = [];
              ...
              if (x.logs && x.logs.length > 0) {
                if (x.logs == error_one || x.logs == error_two) {
                  const report = new FailedReport(
                    ...
                    x.logs,
                    $.cronId,
                    moment($.time).format("DD-MM-YY HH:mm:ss"),
                  );
                  result.push(report);
                }
              }
...
module.exports.AlignProducts = async (product, allCronList, slowCronIds) => {
...
module.exports.GetAlternateProxyProviderId = async (details, sequence) => {
...
module.exports.GetIsStepReached = async (details, stepCount) => {
  if (details.AlternateProxyProvider) {
    const listOfThresholdProvider = _.find(
      details.AlternateProxyProvider,
      (x) => x.Sequence == stepCount,
    );
    return (
...
module.exports.MapAlternateProxyProviderDetails = async (idx, payload) => {
...
module.exports.MapSqlToCronLog = async (runInfo, _idx) => {
  let cronLog = {};
  try {
    cronLog.index = _idx;
...
module.exports.MapLatestPriceInfo = async (scrapeList, focusId) => {
...
module.exports.UpsertProductDetailsInSql = async (payload, mpid, req) => {
  try {
...
/************************* PRIVATE FUNCTIONS *********************/
function getMappedDetails(product, fieldName) {
  let res = "";
  try {
...
function getCommonEntityValue(item, keyType, checkAll) {
  if (checkAll == true) {
...
function getContextItem(product, key) {
...
function mapUserDataToDbData(userData, dbData, AuditInfo) {
