...
/************* PUBLIC APIS *************/
ipHealthController.get(
  "/schedule/ip-health",
  asyncHandler(async (req: Request, res: Response) => {
    if (applicationConfig.START_IP_HEALTH_CRON == false) {
      return res.status(_codes.OK).send(`Ip-Health Cron not started as per settings at ${new Date()}`);
    }
    ...
    if (ipHealthCron) {
      return res.status(_codes.OK).send(`Successfully Started Ip-Health Cron at ${new Date()}`);
    } else {
      return res.status(_codes.BAD_REQUEST).send(`Some error occurred while starting Ip-Health Cron at ${new Date()}`);
    }
  })
);
...
async function checkIpHealthV2() {
  ...
  if (cronSettingsList && cronSettingsList.length > 0) {
    ...
    if (ipList && ipList.length > 0) {
      ...
      if (ipHealthStatus && ipHealthStatus.data && ipHealthStatus.data.status == "SUCCESS") {
        const emailBody = await getEmailBody(ipHealthStatus.data, cronSettingsList);
...
async function getEmailBody(data: any, cronSettingsList: any) {
  let str = '<h2>Hi, Please find the IP health status as of now below.</h2><br/><table border="1"><thead><tr><th scope="col">#</th><th scope="col">Cron Name</th><th scope="col">IP</th><th scope="col">Port</th><th scope="col">Status</th><th scope="col">Ping Response</th></tr></thead><tbody>';
...
async function triggerExceptionEmail(dateTimeStr: any) {
  await TriggerEmail(`<h1>Some error occurred while getting IP health.</h1>`, `IP-HEALTH | ${applicationConfig.ENV_NAME} | EXCEPTION | IP Health Status at ${dateTimeStr}`, applicationConfig.IP_HEALTH_EMAIL_ID);
}
