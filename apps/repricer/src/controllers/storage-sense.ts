...
import { TriggerEmail, TriggerEmailForStorage } from "../middleware/storage-sense-helpers/email-helper";
...
storageSenseController.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    var storageSenseCron = cron.schedule(
      ...
      async () => {
        console.log(`STORAGE-SENSE : Sensing Storage for server at ${new Date()}`);
        await senseAndClean();
      },
      { scheduled: true }
    );
    if (storageSenseCron) {
      return res.status(_codes.OK).send(`Successfully Started Storage-Sense at ${new Date()}`);
    } else {
      return res.status(_codes.BAD_REQUEST).send(`Some error occurred while starting Storage-Sense Cron at ${new Date()}`);
    }
  })
);

storageSenseController.get(
  "/schedule/history-purger",
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (historyPurgerCron) {
      return res.status(_codes.OK).send(`Successfully Started History-Purger at ${new Date()}`);
    } else {
      return res.status(_codes.BAD_REQUEST).send(`Some error occurred while starting History-Purger Cron at ${new Date()}`);
    }
  })
);

storageSenseController.get(
  "/manual/history-purger",
  asyncHandler(async (req: Request, res: Response) => {
    console.log(`HISTORY-PURGER : Purging History at ${new Date()}`);
    await purgeHistory();
    return res.status(_codes.OK).send(`Successfully Started History-Purger at ${new Date()}`);
...
async function senseAndClean() {
  ...
  if (diskDetails.parts && diskDetails.parts.length > 0) {
    ...
    console.log(`STORAGE-SENSE : ${new Date().toISOString()} : FREE SPACE : ${freePercentage} %`);
...
async function purgeHistory() {
    ...
    for (const subDir of listOfSubDirectories) {
      ...
      if (listOfDir && listOfDir.length > 0) {
        var directoriesToRemove = _.remove(listOfDir, ($) => {
          return !listOfDatesToKeep.includes($.substring($.lastIndexOf(applicationConfig.STORAGE_SENSE_FILE_DELIMITER), $.length).replace(applicationConfig.STORAGE_SENSE_FILE_DELIMITER, "").trim());
        });
  ...
  const emailBody = await getEmailBodyForHistoryPurge(_.last(listOfDatesToKeep));
  await TriggerEmail(emailBody, emailSubject, applicationConfig.HISTORY_PURGER_EMAIL_ID);
}
...
const getPreviousDate = (date: any, noOfDays: any) => date.setDate(date.getDate() - noOfDays);
...
async function getEmailBodyForHistoryPurge(lastDate: any) {
  let str = "<h2>Hi, We have purged all History records for all the products.</h2><br/>";
  str += '<span style="color:blue">All records on or before the date <b>' + lastDate + '</b> have been purged.</span><br/><br/>Please restart the server bearing IP : <span style="color:green"><b>' + applicationConfig.ENV_IP + "</b></span>";
  return str;
}
