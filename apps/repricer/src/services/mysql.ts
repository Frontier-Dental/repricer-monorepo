...
export async function UpsertVendorData(payload: any, vendorName: any) {
  ...
  try {
    ...
    if (!payload.qBreakCount) {
      payload.qBreakCount = null;
    }
    if (!payload.qBreakDetails) {
      payload.qBreakDetails = null;
    }
...
export async function UpdateVendorData(payload: any, vendorName: any) {
  ...
  try {
    ...
    if (rows != null && (rows as any)[0] != null) {
      upsertResult = (rows as any)[0][0];
      logger.info(`UPDATE_RESULT for ${payload.mpid}: ${JSON.stringify(upsertResult)}`);
    }
    if (upsertResult && upsertResult != null) {
      return upsertResult["updatedIdentifier"];
    } else return null;
...
export async function UpdateProductV2(mpid: any, itemData: any, tId: any, fId: any, mId: any) {
  ...
  try {
    ...
    logger.info(`Updated in DB for ${mpid} with records ${noOfRecords}`);
    return noOfRecords;
  } finally {
    SqlConnectionPool.releaseConnection(db);
  }
...
export async function MapVendorToRoot(data: any) {
  ...
  try {
    ...
    logger.debug(`MapVendorToRoot result: ${(noOfRecords[0] as any)[0]}`);
