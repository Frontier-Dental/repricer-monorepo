...
import * as SqlMapper from "../mapper/mysql-mapper";
...
const mockKnexChain: {
  ...
  where: jest.fn(function (a: unknown, _b?: unknown, _c?: unknown) {
    if (typeof a === "function") {
      const ctx = { where: jest.fn().mockReturnThis(), orWhere: jest.fn().mockReturnThis() };
      a.call(ctx);
    }
...
// --- Mock: config (keys used by utility/mysql.ts)
jest.mock("../config", () => ({
  applicationConfig: {
...
describe("utility/mysql", () => {
  ...
  describe("GetLatestRunInfo", () => {
    it("calls knex raw with SP and params, returns result[0]", async () => {
      ...
      const result = await mysqlUtility.GetLatestRunInfo(5, "2024-01-01", "2024-01-02");
  ...
  describe("GetLatestRunInfoForCron", () => {
    it("calls knex raw with cron SP and params", async () => {
      ...
      const result = await mysqlUtility.GetLatestRunInfoForCron(5, "2024-01-01", "2024-01-02", "cron-1");
  ...
  describe("GetRecentInProgressScrapeOnlyRuns", () => {
    it("calls knex raw with in-progress SP, no params", async () => {
      ...
      const result = await mysqlUtility.GetRecentInProgressScrapeOnlyRuns();
  ...
  describe("GetNumberOfScrapeProducts", () => {
    it("gets connection, executes count query, returns count, releases connection", async () => {
      ...
      const result = await mysqlUtility.GetNumberOfScrapeProducts();
  ...
  describe("GetScrapeProductList", () => {
    it("calls SP with pageNumber and pageSize, returns first result set", async () => {
      ...
      const result = await mysqlUtility.GetScrapeProductList(1, 10);
  ...
  describe("GetScrapeProductListByFilter", () => {
    it("calls filter SP with pageSize, filterText, pageNumber", async () => {
      ...
      const result = await mysqlUtility.GetScrapeProductListByFilter("filter", 20, 2);
  ...
  describe("GetAllScrapeProductDetails", () => {
    it("calls get-all SP and returns first result set", async () => {
      ...
      const result = await mysqlUtility.GetAllScrapeProductDetails();
  ...
  describe("UpsertProductDetails", () => {
    it("calls upsert SP with payload fields and returns first result", async () => {
      ...
      const result = await mysqlUtility.UpsertProductDetails(payload);
  ...
  describe("DeleteScrapeProductById", () => {
    it("executes delete by MpId and returns result", async () => {
      ...
      const result = await mysqlUtility.DeleteScrapeProductById("mp-123");
  ...
  describe("GetLastScrapeDetailsById", () => {
    it("calls SP with mpId and returns first result set", async () => {
      ...
      const result = await mysqlUtility.GetLastScrapeDetailsById("mp1");
  ...
  describe("UpsertVendorData", () => {
    ...
    it("TRADENT: sets contextSpName and returns updatedIdentifier", async () => {
      ...
      const result = await mysqlUtility.UpsertVendorData({ ...basePayload }, "TRADENT");
    ...
    it("FRONTIER: uses FRONTIER SP", async () => {
      ...
      const result = await mysqlUtility.UpsertVendorData({ ...basePayload }, "FRONTIER");
    ...
    it("MVP, TOPDENT, FIRSTDENT, TRIAD, BITESUPPLY: use correct SP names", async () => {
      ...
      for (let i = 0; i < vendors.length; i++) {
        ...
        const result = await mysqlUtility.UpsertVendorData({ ...basePayload }, vendors[i]);
        expect(mockConnection.query).toHaveBeenCalledWith(expect.stringContaining(spNames[i]), expect.any(Array));
        expect(result).toBe(i);
      }
    });

    it("defaults missing payload fields (inventoryThreshold, percentageDown, etc.)", async () => {
      ...
      const result = await mysqlUtility.UpsertVendorData(minimal, "TRADENT");
    ...
    it("unknown vendor: contextSpName stays null, still runs query and release", async () => {
      mockConnection.query.mockResolvedValue([[{ 0: { updatedIdentifier: 0 } }]]);

      await mysqlUtility.UpsertVendorData({ ...basePayload }, "UNKNOWN");
  ...
  describe("UpsertProductDetailsV2", () => {
    it("calls V4 SP with payload and returns first result", async () => {
      ...
      const result = await mysqlUtility.UpsertProductDetailsV2(payload);
  ...
  describe("GetCompleteProductDetails", () => {
    it("calls get-all SP and maps result with SqlMapper", async () => {
      ...
      const result = await mysqlUtility.GetCompleteProductDetails();
  ...
  describe("GetNumberOfRepriceEligibleProductCount", () => {
    it("returns totalCount - nullCount from knex", async () => {
      ...
      const result = await mysqlUtility.GetNumberOfRepriceEligibleProductCount();
  ...
  describe("GetAllRepriceEligibleProductByFilter", () => {
    it("returns empty array when no paginated MpIds", async () => {
      ...
      const result = await mysqlUtility.GetAllRepriceEligibleProductByFilter(0, 10);
    ...
    it("fetches paginated MpIds, builds union, maps result", async () => {
      ...
      const result = await mysqlUtility.GetAllRepriceEligibleProductByFilter(1, 5);
  ...
  describe("GetAllRepriceEligibleProductByTag", () => {
    it("returns empty array when no matching MpIds", async () => {
      ...
      const result = await mysqlUtility.GetAllRepriceEligibleProductByTag(null, null);
    ...
    it("with mpId and channelId builds union and maps", async () => {
      ...
      const result = await mysqlUtility.GetAllRepriceEligibleProductByTag("mp1", "ch1");

      expect(result).toEqual([{ mpId: "mp1" }]);
    });

    it("with only channelId uses channelIdSearch wildcard", async () => {
      mockKnexChain.union.mockImplementation(() => mockKnexChain);
      mockKnexChain.distinct.mockResolvedValue([{ MpId: "mp1" }]);
      mockKnexChain.orderBy.mockResolvedValue([{ ProductId: "mp1" }]);
      (SqlMapper.MapProductDetailsList as jest.Mock).mockReturnValue([{ mpId: "mp1" }]);

      await mysqlUtility.GetAllRepriceEligibleProductByTag(null, "ch1");

      expect(mockKnexChain.distinct).toHaveBeenCalled();
    });

    it("with only mpId uses mpIdSearch exact match", async () => {
      mockKnexChain.union.mockImplementation(() => mockKnexChain);
      mockKnexChain.distinct.mockResolvedValue([{ MpId: "mp1" }]);
      mockKnexChain.orderBy.mockResolvedValue([{ ProductId: "mp1" }]);
      (SqlMapper.MapProductDetailsList as jest.Mock).mockReturnValue([{ mpId: "mp1" }]);

      await mysqlUtility.GetAllRepriceEligibleProductByTag("mp1", null);
  ...
  describe("GetFullProductDetailsById", () => {
    it("calls SP with mpid and maps result", async () => {
      ...
      const result = await mysqlUtility.GetFullProductDetailsById("mp1");
  ...
  describe("UpdateVendorData", () => {
    ...
    it("TRADENT: calls update SP and returns updatedIdentifier", async () => {
      ...
      const result = await mysqlUtility.UpdateVendorData(updatePayload, "TRADENT");
    ...
    it("all vendor names use correct update SP", async () => {
      ...
      for (let i = 0; i < vendors.length; i++) {
        mockConnection.query.mockResolvedValue([[{ 0: { updatedIdentifier: i } }]]);
        const result = await mysqlUtility.UpdateVendorData(updatePayload, vendors[i]);
        expect(mockConnection.query).toHaveBeenCalledWith(expect.stringContaining(spNames[i]), expect.any(Array));
        expect(result).toBe(i);
      }
    });
    ...
    it("defaults missing fields and returns null when no result", async () => {
      ...
      const result = await mysqlUtility.UpdateVendorData(minimal, "TRADENT");

      expect(result).toBeNull();
    });

    it("unknown vendor: contextSpName null, still releases connection", async () => {
      mockConnection.query.mockResolvedValue([[{ 0: { updatedIdentifier: 0 } }]]);

      await mysqlUtility.UpdateVendorData(updatePayload, "UNKNOWN");
  ...
  describe("GetLinkedVendorDetails", () => {
    it("TRADENT: queries table_tradentDetails and returns Id", async () => {
      ...
      const result = await mysqlUtility.GetLinkedVendorDetails("mp1", "TRADENT");
    ...
    it("FRONTIER, MVP, TOPDENT, FIRSTDENT, TRIAD, BITESUPPLY: correct table", async () => {
      ...
      for (const [vendor, table] of cases) {
        ...
        await mysqlUtility.GetLinkedVendorDetails("mp1", vendor);
  ...
  describe("UpdateProductV2", () => {
    it("executes update with itemData and vendor ids", async () => {
      ...
      const result = await mysqlUtility.UpdateProductV2("1", itemData, 10, 20, 30);
  ...
  describe("ChangeProductActivation", () => {
    it("updates all vendor tables and returns last result", async () => {
      ...
      const result = await mysqlUtility.ChangeProductActivation("1", 1);
  ...
  describe("MapVendorToRoot", () => {
    it("fetches vendor ids, then updates scrape product list", async () => {
      ...
      const result = await mysqlUtility.MapVendorToRoot({
        MPID: "123",
        CronName: " CronName ",
        CronId: 5,
      });
  ...
  describe("ToggleDataScrapeForId", () => {
    it("updates IsActive, LastUpdatedBy, LastUpdatedAt by MpId", async () => {
      ...
      const result = await mysqlUtility.ToggleDataScrapeForId("1", 1, {
        UpdatedBy: "user",
  ...
  describe("UpdateBranchDataForVendor", () => {
    it("TRADENT: updates table_tradentDetails with payload", async () => {
      ...
      const result = await mysqlUtility.UpdateBranchDataForVendor("1", "TRADENT", payLoad);
    ...
    it("FRONTIER, MVP, FIRSTDENT, TOPDENT, TRIAD, BITESUPPLY: correct table", async () => {
      ...
      for (let i = 0; i < vendors.length; i++) {
        mockConnection.execute.mockResolvedValue([[{ affectedRows: 1 }]]);
        await mysqlUtility.UpdateBranchDataForVendor("1", vendors[i], payLoad);
  ...
  describe("ExecuteQuery", () => {
    it("gets connection, executes query with params, releases", async () => {
      ...
      const result = await mysqlUtility.ExecuteQuery("SELECT 1", [1]);
  ...
  describe("CreateUser", () => {
    it("inserts username and password into users and returns userId", async () => {
      ...
      const result = await mysqlUtility.CreateUser("alice", "hashed");
  ...
  describe("AuthenticateUser", () => {
    it("returns user without password when password matches", async () => {
      ...
      const result = await mysqlUtility.AuthenticateUser("alice", "secret");
    ...
    it("returns null when user not found", async () => {
      ...
      const result = await mysqlUtility.AuthenticateUser("nobody", "pwd");
    ...
    it("returns null when password invalid", async () => {
      ...
      const result = await mysqlUtility.AuthenticateUser("alice", "wrong");
  ...
  describe("ChangePassword", () => {
    it("returns true when update affects rows", async () => {
      ...
      const result = await mysqlUtility.ChangePassword("alice", "newhash");
  ...
  describe("CheckUserExists", () => {
    it("returns user when found", async () => {
      ...
      const result = await mysqlUtility.CheckUserExists("alice");
    ...
    it("returns null when not found", async () => {
      ...
      const result = await mysqlUtility.CheckUserExists("nobody");
  ...
  describe("GetAllRepriceEligibleProductByMpid", () => {
    it("calls SP, maps result and returns", async () => {
      ...
      const result = await mysqlUtility.GetAllRepriceEligibleProductByMpid("mp1");
  ...
  describe("GetAllRepriceEligibleProductByChannelId", () => {
    it("calls SP by channel id and maps result", async () => {
      ...
      const result = await mysqlUtility.GetAllRepriceEligibleProductByChannelId("ch1");
    ...
    it("on exception logs and returns mapper result for null", async () => {
      ...
      await mysqlUtility.GetAllRepriceEligibleProductByChannelId("ch1");
